...
from database_manager_4G import DatabaseManager
import sys
import subprocess
from ttkthemes import ThemedTk
import logging
...
from clases_modelos import Licitacion, Lote, Documento, Oferente, Empresa, Riesgo
from database_manager_4G import DatabaseManager
try:
    import openpyxl
...
from clases_modelos import Licitacion, Lote, Documento, Oferente, Empresa, Riesgo
from database_manager_4G import DatabaseManager
from report_generator import ReportGenerator # <-- AÑADE ESTA LÍNEA
...
class VentanaResultadosTests(tk.Toplevel):
    ...
    def __init__(self, parent, results_log):
        ...
        ttk.Button(btn_frame, text="💾 Exportar a .txt", command=self._exportar).pack(side=tk.LEFT)
...
class VentanaAnalisisPaquetes(tk.Toplevel):
    ...
    def _crear_tabla_pivote(self, parent_frame):
        ...
        # Configurar el tag para resaltar las ofertas mínimas
        ...
        for lote_num, ofertas in sorted(matriz_ofertas.items()):
            # Encontrar el monto mínimo para este lote
            ...
            # Aplicar el resaltado a la celda con el monto mínimo
            for i, oferente in enumerate(todos_los_oferentes):
                if ofertas.get(oferente) and ofertas[oferente]['monto'] == monto_minimo:
                    # El Treeview no permite colorear celdas individuales, así que se aplica a toda la fila
                    # pero el texto en negrita da la pista visual. Es una limitación de ttk.Treeview.
    ...
    def _mostrar_resumen(self, parent_frame):
        ...
        # Resultado del Paquete Individual
        ...
        if paquete_unico:
            ...
            monto_uni = float('inf') # Un valor infinito si no hay paquete único
